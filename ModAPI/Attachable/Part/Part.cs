...
namespace eightyseven.ModApi.Attachable
{
    ...
    public class Part : MonoBehaviour, IHasBolts
    {
        ...
        IEnumerator initPartCoroutine(PartSaveInfo saveInfo, TriggerData triggerData, PartSettings partSettingsRef = default, Bolt[] boltRefs = null)
        {
            while (ModClient.getBoltManager == null)
            {
                yield return new WaitForSeconds(0.5f);
                Debug.Log(gameObject.name + ": Waiting on bolt manager");
            }

            while (!ModClient.getBoltManager.assetsLoaded)
            {
                yield return new WaitForSeconds(0.5f);
                Debug.Log(gameObject.name + ": Waiting for bolt manager assets");
            }

            Debug.Log("Loading part: " + gameObject.name);

            validiatePart(partSettingsRef);
            ...
            if (boltRefs != null)
            {
                Debug.Log(gameObject.name + ": Loading bolts");
                initBolts(boltRefs);
                while (!_boltsInited)
                {
                    yield return new WaitForSeconds(0.5f);
                    Debug.Log("Waiting for bolts to be initialized");
                }
                initBolts();
                Debug.Log(gameObject.name + ": Bolts loaded");
            }
        ...
        {
            StartCoroutine(initPartCoroutine(saveInfo, triggerData, partSettingsRef, boltRefs));
        }
        ...
        /// </summary>
        protected virtual void validiatePart(PartSettings partSettings)
        {
            // Written, 25.09.2021
