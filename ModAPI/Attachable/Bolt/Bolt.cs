...
namespace TommoJProductions.ModApi.Attachable
{
    ...
    public class Bolt
    {
        ...
        protected virtual void setupBoltModel()
        {
            ...
            Debug.Log("setupBoltModel");

            if (settings.customPrefab != null)
            {
                _model = GameObject.Instantiate(settings.customPrefab);
            }
            else
            {
                ...
                {
                    case BoltType.nut:
                        _model = GameObject.Instantiate(ModClient.getBoltManager.nutPrefab);
                        break;
                    case BoltType.screw:
                        _model = GameObject.Instantiate(ModClient.getBoltManager.screwPrefab);
                        break;
                    case BoltType.longBolt:
                        _model = GameObject.Instantiate(ModClient.getBoltManager.longBoltPrefab);
                        break;
                    default:
                    case BoltType.shortBolt:
                        _model = GameObject.Instantiate(ModClient.getBoltManager.shortBoltPrefab);
                        break;
                }
            }
        ...
        IEnumerator activateBoltCoroutine(bool active)
        {
            while (model == null)
            {
                yield return new WaitForSeconds(0.5f);
                //Debug.Log("Waiting for bolt model");
            }
        ...
        public virtual void activateBolt(bool active) 
        {
            ...
            LevelManager.Instance.StartCoroutine(activateBoltCoroutine(active));
        }
        /// <summary>
